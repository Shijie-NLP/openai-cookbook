...
print(generate_summaries(df.iloc[0]))
...
def evaluate_summaries(row):
    simple_messages = [
        {
            "role": "user",
            "content": evaluation_prompt.format(original_article=row["content"], summary=row["simple_summary"]),
        }
    ]
    complex_messages = [
        {
            "role": "user",
            "content": evaluation_prompt.format(original_article=row["content"], summary=row["complex_summary"]),
        }
    ]

    simple_summary = client.chat.completions.parse(model="gpt-4o", messages=simple_messages, response_format=ScoreCard)
    simple_summary = simple_summary.choices[0].message.parsed

    complex_summary = client.chat.completions.parse(
        model="gpt-4o", messages=complex_messages, response_format=ScoreCard
    )
...
avg_scores_df = pd.DataFrame(
    {"Criteria": criteria, "Original Prompt": simple_avg_scores, "Improved Prompt": complex_avg_scores}
)
