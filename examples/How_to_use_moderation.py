...
async def get_chat_response(user_request):
    ...
    response = client.chat.completions.create(model=GPT_MODEL, messages=messages, temperature=0.5)
...
async def execute_chat_with_input_moderation(user_request):
    ...
    while True:
        ...
        done, _ = await asyncio.wait([moderation_task, chat_task], return_when=asyncio.FIRST_COMPLETED)
        ...
        if moderation_task.result() is True:
            chat_task.cancel()
...
async def get_results(request):
    results = await execute_chat_with_input_moderation(request)
    return results


# Call the main function with the good request - this should go through
print(asyncio.run(get_results(good_request)))
...
print(asyncio.run(get_results(bad_request)))


# Looks like our moderation worked - the first question was allowed through, but the second was blocked for inappropriate content. Here is a similar example that works with images.
...
def check_image_moderation(image_url):
    response = client.moderations.create(
        model="omni-moderation-latest", input=[{"type": "image_url", "image_url": {"url": image_url}}]
    )
...
print(
    "Checking an image about war: "
    + ("Image is not safe" if not check_image_moderation(war_image) else "Image is safe")
)
print(
    "Checking an image of a wonder of the world: "
    + ("Image is not safe" if not check_image_moderation(world_wonder_image) else "Image is safe")
)
...
async def execute_all_moderations(user_request):
    ...
    while True:
        done, _ = await asyncio.wait([input_moderation_task, chat_task], return_when=asyncio.FIRST_COMPLETED)
        ...
        if input_moderation_task.result() is True:
            chat_task.cancel()
        ...
        if chat_task in done:
            ...
            if output_moderation_response is True:
...
async def get_all_results(test):
    results = await execute_all_moderations(test)
    return results
...
for test in tests:
    print(test)
    print(asyncio.run(get_all_results(test)))
    print("\n\n")
