...
def analyze_image(data_uri):
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": [{"type": "image_url", "image_url": {"url": f"{data_uri}"}}]},
        ],
        max_tokens=500,
        temperature=0,
        top_p=0.1,
...
for f in files:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        ...
        futures = [executor.submit(analyze_doc_image, img) for img in imgs[1:]]
...
def get_embeddings(text):
    embeddings = client.embeddings.create(model="text-embedding-3-small", input=text, encoding_format="float")
