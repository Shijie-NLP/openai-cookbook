...
print(encoding.decode([83, 8251, 2488, 382, 2212, 0]))
...
print([encoding.decode_single_token_bytes(token) for token in [83, 8251, 2488, 382, 2212, 0]])
...
for model in ["gpt-3.5-turbo", "gpt-4-0613", "gpt-4", "gpt-4o", "gpt-4o-mini"]:
    print(model)
    ...
    if len(functions) > 0:
        for f in functions:
            ...
            if len(function["parameters"]["properties"]) > 0:
                func_token_count += prop_init  # Add tokens for the start of each property
...
for model in ["gpt-3.5-turbo", "gpt-4", "gpt-4o", "gpt-4o-mini"]:
    ...
    # example token count from the OpenAI API
    response = client.chat.completions.create(model=model, messages=example_messages, tools=tools, temperature=0)
    print(f"{response.usage.prompt_tokens} prompt tokens counted by the OpenAI API.")
    print()
