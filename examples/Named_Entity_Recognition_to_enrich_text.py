...
def find_all_links(label_entities: dict) -> dict:
    ...
    return {e: find_link(e) for label, entities in label_entities.items() for e in entities if label in whitelist}
    ...
    return [
        {
            ...
            "function": {
                ...
                "parameters": {
                    "type": "object",
                    "properties": {
                        f"r'^(?:' + '|'.join({labels}) + ')$'": {"type": "array", "items": {"type": "string"}}
                    },
