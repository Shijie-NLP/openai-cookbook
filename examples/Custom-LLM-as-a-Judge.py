...
SELECT *
FROM 'hf://datasets/stanfordnlp/coqa/data/validation-00000-of-00001.parquet'
LIMIT 5
""").fetchall()

single_result = full_result[0]
...
hallucinated_answers = asyncio.run(asyncio.gather(*[hallucinate_answer(qa) for qa in qa_pairs]))
...
print(qa_pairs[0].question, "On a correct answer:", qa_pairs[0].generated_answer)
print(
    asyncio.run(
        numeric_rater(
            qa_pairs[0].question,
            qa_pairs[0].generated_answer,
            qa_pairs[0].expected_answer,
        )
    )
)

print(hallucinations[0].question, "On a hallucinated answer:", hallucinations[0].generated_answer)
print(
    asyncio.run(
        numeric_rater(
            hallucinations[0].question,
            hallucinations[0].generated_answer,
            hallucinations[0].expected_answer,
        )
    )
)
...
def data():
    for pair in hallucinations:
        yield {"input": asdict(pair), "expected": 0, "metadata": {"hallucination": True}}
