...
async def get_chat_response(user_request):
    ...
    response = openai.chat.completions.create(model=GPT_MODEL, messages=messages, temperature=0.5)
...
async def topical_guardrail(user_request):
    ...
    response = openai.chat.completions.create(model=GPT_MODEL, messages=messages, temperature=0)
...
async def execute_chat_with_guardrail(user_request):
    ...
    while True:
        done, _ = await asyncio.wait([topical_guardrail_task, chat_task], return_when=asyncio.FIRST_COMPLETED)
...
async def execute_with_request(request):
    results = await execute_chat_with_guardrail(request)
    return results


# Call the main function with the good request - this should go through
print(asyncio.run(execute_with_request(good_request)))
...
print(asyncio.run(execute_with_request(bad_request)))
...
async def moderation_guardrail(chat_response):
    ...
    response = openai.chat.completions.create(model=GPT_MODEL, messages=mod_messages, temperature=0)
...
async def get_results(test):
    results = await execute_all_guardrails(test)
    return results
...
for test in tests:
    print(asyncio.run(get_results(test)))
    print("\n\n")
