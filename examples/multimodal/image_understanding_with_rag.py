...
cache_path = f".local_cache/{hashlib.md5(prompt.encode('utf-8')).hexdigest()}.png"
...
Path(".local_cache/images").mkdir(parents=True, exist_ok=True)

urlretrieve(
    "https://raw.githubusercontent.com/robtinn/image_understanding_rag_dataset/main/data/df.csv", ".local_cache/df.csv"
)

urlretrieve(
    "https://raw.githubusercontent.com/robtinn/image_understanding_rag_dataset/main/data/images/1.png",
    ".local_cache/images/1.png",
)
urlretrieve(
    "https://raw.githubusercontent.com/robtinn/image_understanding_rag_dataset/main/data/images/2.png",
    ".local_cache/images/2.png",
)
urlretrieve(
    "https://raw.githubusercontent.com/robtinn/image_understanding_rag_dataset/main/data/images/3.png",
    ".local_cache/images/3.png",
)
urlretrieve(
    "https://raw.githubusercontent.com/robtinn/image_understanding_rag_dataset/main/data/images/4.png",
    ".local_cache/images/4.png",
)
urlretrieve(
    "https://raw.githubusercontent.com/robtinn/image_understanding_rag_dataset/main/data/images/5.png",
    ".local_cache/images/5.png",
)
urlretrieve(
    "https://raw.githubusercontent.com/robtinn/image_understanding_rag_dataset/main/data/images/6.png",
    ".local_cache/images/6.png",
)
urlretrieve(
    "https://raw.githubusercontent.com/robtinn/image_understanding_rag_dataset/main/data/images/7.png",
    ".local_cache/images/7.png",
)
...
text_vector_store = client.vector_stores.create(
    ...
    metadata={"purpose": "text_understanding", "created_by": "notebook", "version": "1.0"},
)
...
text_image_vector_store = client.vector_stores.create(
    ...
    metadata={"purpose": "text_image_understanding", "created_by": "notebook", "version": "1.0"},
)
...
def upload_files_to_vector_store(vector_store_id, df, column_name="full_sentiment"):
    ...
    for i, row in tqdm(df.iterrows(), total=len(df), desc="Uploading context files"):
        ...
        file = client.vector_stores.files.upload(vector_store_id=vector_store_id, file=file_stream)
        file_ids.append(file.id)

    for i, row in tqdm(df.iterrows(), total=len(df), desc="Updating file attributes"):
        client.vector_stores.files.update(
            vector_store_id=vector_store_id, file_id=file_ids[i], attributes={"month": row["month"]}
        )
...
query = "Were there any comments about the 'spaghetti'?"
print(f"🔍 Query: {query}\n")
...
IMAGE_ID_MAPPING = {
    f"context_{row['id']}_{row['month']}.txt": row["image_path"] for _, row in df[~df["image_path"].isna()].iterrows()
}
    ...
    tools=[
        {
            "type": "file_search",
            "vector_store_ids": [text_image_vector_store_id],
            "filters": {"type": "eq", "key": "month", "value": "june"},
        }
    ],
)
...
def prepare_evaluation_data(df: pd.DataFrame, text_col: str = "full_sentiment", label_col: str = "label") -> list:
    """
    ...
    """
    return [{"item": {"input": str(row[text_col]), "ground_truth": row[label_col]}} for _, row in df.iterrows()]
...
def create_eval_run(evaluation_data: list, eval_id: str) -> str:
    ...
    eval_config = {
        ...
        "source": {"type": "file_content", "content": evaluation_data},
    }

    run = client.evals.runs.create(eval_id=eval_id, data_source=eval_config)
...
eval_obj = client.evals.create(
    ...
    data_source_config={
        ...
        "item_schema": {
            "type": "object",
            "properties": {"input": {"type": "string"}, "ground_truth": {"type": "string"}},
...
failed_samples = [
    {"Input": sample["sample"]["input"], "Model Output": sample["sample"]["output"]}
...
print(failed_df.style.set_properties(**{"text-align": "left"}))
...
deleted_vector_store = client.vector_stores.delete(vector_store_id=text_vector_store_id)
print(deleted_vector_store)

deleted_vector_store = client.vector_stores.delete(vector_store_id=text_image_vector_store_id)
print(deleted_vector_store)
