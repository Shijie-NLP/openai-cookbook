...
def delivery_exception_support_handler(test_image: str):
    ...
    payload["messages"] = [
        ...
        },
        {
            "role": "user",
            "content": [
                {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{image_data[test_image]}"}},
            ],
        },
    ]
    function_calls = instructor.from_openai(OpenAI(), mode=instructor.Mode.PARALLEL_TOOLS).chat.completions.create(
        **payload
    )
